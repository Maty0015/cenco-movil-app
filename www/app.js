...
async function dispararSOSInstitucional() {
    ...
    const enviarDatosSupabase = async (textoDireccion, latitudVal, longitudVal) => {
        ...
        if (error) {
            ...
            // ✅ Activar Pantalla Camuflada tras 1.5 segundos (para permitir ver el toast de éxito inicialmente)
            setTimeout(() => {
                window.activarPantallaCamuflada();
            }, 1500);
        }
    };
...
/**
 * 🕶️ Funciones y Eventos para la Pantalla Camuflada (Falso Apagado)
 */
window.activarPantallaCamuflada = function() {
    const el = document.getElementById('pantalla-camuflada');
    if (el) {
        el.style.display = 'block';
        console.log("🕶️ Pantalla camuflada activada. Presione dos veces para desactivar.");
    }
};

const pantallaCamuflada = document.getElementById('pantalla-camuflada');
let ultimoToqueCamuflaje = 0;

if (pantallaCamuflada) {
    const detectarDobleToque = (e) => {
        e.preventDefault();
        const ahora = Date.now();
        const diferencia = ahora - ultimoToqueCamuflaje;
        if (diferencia > 0 && diferencia < 300) {
            pantallaCamuflada.style.display = 'none';
            window.mostrarNotificacionToast("🔓", "Pantalla Restaurada", "La aplicación ha vuelto a su estado normal.");
        }
        ultimoToqueCamuflaje = ahora;
    };
